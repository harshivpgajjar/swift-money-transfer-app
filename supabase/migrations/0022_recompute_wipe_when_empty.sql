-- When a (retailer, account) has NO remaining activity (e.g. its cash-book
-- entries were replaced away), recompute_balances used to return early and
-- leave stale daily_balances rows behind. Wipe them instead.
...
begin
  if p_from_date is null then
    ...
    if v_from is null then
      -- no activity at all → no balances should exist
