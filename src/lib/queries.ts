...
export type RetailerSummary = {
  ...
  opening?: number; // carried balance at window start (getRetailerSummariesByDate)
  total_transferred: number;
  total_reversed: number;
  total_cash: number;
  outstanding: number;
  // Defaulter handling (populated by getRetailerSummariesByDate).
  defaulted?: boolean;
  atRisk?: boolean; // dues > 0 and no approved payment in 45 days
  // Owner/internal account (Pankaj, Harshiv) — carved out of receivable totals.
  personal?: boolean;
};
...
export async function getRetailerHistory(
  ...
  if (range) {
    // money_requests has no txn_date — filter on created_at (UTC, matching todayIso()).
    reqQ = reqQ
...
export type ActionRow = {
  ...
  phone: string | null;
  fos_id: string | null;
  outstanding: number; // pending till 3 PM (old + by-3PM − collected)
...
/* Daily collection to-do, classified server-side (cap-proof, FOS-scoped).
   Buckets: attention (transfer by 3 PM uncleared) · alert (owed at last close,
   uncleared) · atrisk (45d) · defaulter. */
...
export async function getActionBadge(distributorId: string, fosId?: string): Promise<number> {
  const rows = await getActionCenter(distributorId, fosId);
...
/* Outstanding screen, windowed: per-retailer MOVEMENTS (transferred/reversed/
   ...
   is the true carried balance — the closing of the latest day on/before
   range.to. This keeps the default light (e.g. yesterday's movements) while the
   outstanding total still reconciles like the per-account views. */
export async function getRetailerSummariesByDate(
  distributorId: string,
  accountId: string,
  range: DateRange,
  ...
  let rQ = supabase
    .from("profiles")
    .select(`id, retailer_code, full_name, active, needs_assignment, fos_id, defaulted, personal,
             fos:fos_id(full_name)`)
    .eq("role", "retailer")
    .eq("distributor_id", distributorId)
    .eq("excluded", false)
  ...
  const { data: retailers, error: rErr } = await rQ;
  if (rErr) throw rErr;
  if (!retailers || retailers.length === 0) return [];
  ...
  const [balsRes, cashRes] = await Promise.all([
    supabase
      ...
      .order("balance_date", { ascending: false }),
    // Last cash across ALL accounts (cash book / EOD), newest first. This
    // distributor records collections in daily_balances.cash_received, NOT the
    // in-app cash_submissions flow — so the 45-day "at-risk" signal reads here.
    supabase
      .from("daily_balances")
  ...
  const closing = new Map<string, number>(); // latest closing on/before range.to
  ...
  return retailers.map((r) => {
    ...
    return {
      ...
      defaulted,
      personal,
      // At-risk: owes money and hasn't paid in 45+ days (not already a defaulter).
...
export async function getRetailerDailyBalances(
  ...
  let q = supabase
    ...
    .order("balance_date", { ascending: false });
  q = range
    ? q.gte("balance_date", range.from).lte("balance_date", range.to)
    : q.limit(limit);
  const { data, error } = await q;
  if (error) throw error;
  return data ?? [];
